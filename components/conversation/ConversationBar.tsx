...
export default function ConversationBar({ mode, state, theme }: { mode: 'chat' | 'prompt' | 'instruct' | 'chain', state: AGiXTState, theme: Theme }) {
    ...
    const PromptAgent = async (
        ...
        const promptName = state.chatConfig.promptConfig.promptName + (state.chatConfig.promptConfig.promptName == 'Chat with Commands' && state.chatState.hasFiles ? ' with Files' : '');
        const skipArgs = [
